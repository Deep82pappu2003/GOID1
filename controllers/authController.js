...
module.exports.registerRider = async function(req, res) {
  try {
    ...
    if (!eName || !eId || !eEmail || !contact || !eAddress ||!latitude||!longitude|| !password ) {
    ...
    let token = generateToken(newRider);
    res.cookie("token", token, { httpOnly: true ,maxAge:120000});
    req.session.isLoggedIn = true;
    req.session.user = newRider;
    req.session.homeLocation = { latitude, longitude }; 
